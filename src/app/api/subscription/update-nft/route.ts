...
/**
 * Update subscription based on NFT ownership
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { walletAddress, hasLegionNFT, nftContract, networkName } = body;
    ...
    if (!walletAddress || !nftContract) {
      return addCorsHeaders(
        ApiResponseBuilder.error(
          "MISSING_FIELDS",
          "Wallet address and NFT contract required"
        ),
        request
      );
    }
    ...
    if (existingSubscription.rows.length > 0) {
      ...
      await db.query(
        ...
        [walletAddress, nftContract, networkName, userId, "lifetime_legion"]
      );
    ...
    console.error("❌ NFT subscription update error:", error);
